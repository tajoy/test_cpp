...
class ConfigurationTypeHelper : base::StaticClass
{
    public:
        ...
        static const char* convertToString ( ConfigurationType configurationType )
        {
            ...
            if ( configurationType == ConfigurationType::ToStandardOutput ) return
                    "TO_STANDARD_OUTPUT";
            if ( configurationType == ConfigurationType::MillisecondsWidth ) return
                    "MILLISECONDS_WIDTH";
            ...
            if ( configurationType == ConfigurationType::MaxLogFileSize ) return
                    "MAX_LOG_FILE_SIZE";
            if ( configurationType == ConfigurationType::LogFlushThreshold ) return
                    "LOG_FLUSH_THRESHOLD";
            return "UNKNOWN";
        }
...
static const int kTimeFormatsCount                           =      sizeof (
            kTimeFormats ) / sizeof ( kTimeFormats[0] );
const struct {
    int numb;
    const char* name;
    const char* brief;
...
static const int kCrashSignalsCount                          =      sizeof (
            kCrashSignals ) / sizeof ( kCrashSignals[0] );
}  // namespace consts
}  // namespace base
...
class File : base::StaticClass
{
    public:
        ...
        static inline bool pathExists ( const char* path, bool considerFile = false )
        {
            ...
            return considerFile ? true : ( ( fileType & FILE_ATTRIBUTE_DIRECTORY ) == 0 ?
                                           false : true );
#endif  // ELPP_OS_UNIX
        }
...
class Str : base::StaticClass
{
    public:
                                                ...
                                                const std::string& replaceWith )
        {
            ...
            while ( ( foundAt = str.find ( replaceWhat,
                                           foundAt + 1 ) ) != std::string::npos ) {
                                             ...
                                             const base::type::string_t& replaceWith )
        {
            ...
            while ( ( foundAt = str.find ( replaceWhat,
                                           foundAt + 1 ) ) != base::type::string_t::npos ) {
...
class OS : base::StaticClass
{
    public:
        ...
        static std::string getEnvironmentVariable ( const char* variableName,
                const char* defaultVal, const char* alternativeBashCommand = nullptr )
        {
#if ELPP_OS_UNIX
...
#   endif  // ELPP_COMPILER_MSVC || defined(_MSC_EXTENSIONS)
        ...
        static inline std::string getDateTime ( const char* format,
                ...
                base::TimestampUnit timestampUnit )
        {
            ...
            base::type::EnumType start = static_cast<base::type::EnumType>
                                         ( timestampUnit );
        ...
        static inline unsigned long long getTimeDifference ( const struct timeval&
                endTime, const struct timeval& startTime, base::TimestampUnit timestampUnit )
        {
            if ( timestampUnit == base::TimestampUnit::Microsecond ) {
                return static_cast<unsigned long long> ( static_cast<unsigned long long>
                        ( 1000000 * endTime.tv_sec + endTime.tv_usec ) -
            ...
            for ( ; *format; ++format ) {
                if ( *format == base::consts::kFormatSpecifierChar ) {
                    ...
                    case 'a':  // Day of week (short)
                        buf = base::utils::Str::addToBuff ( base::consts::kDaysAbbrev[tInfo->tm_wday],
                                                            buf, bufLim );
                        continue;
                    case 'A':  // Day of week (long)
                        buf = base::utils::Str::addToBuff ( base::consts::kDays[tInfo->tm_wday], buf,
                                                            bufLim );
                        continue;
                    case 'M':  // month
                        ...
                        continue;
                    case 'b':  // month (short)
                        buf = base::utils::Str::addToBuff ( base::consts::kMonthsAbbrev[tInfo->tm_mon],
                                                            buf, bufLim );
                        continue;
                    case 'B':  // month (long)
                    ...
                    case 'y':  // year (two digits)
                        buf = base::utils::Str::convertAndAddToBuff ( tInfo->tm_year +
                                base::consts::kYearBase, 2, buf, bufLim );
                        continue;
                    case 'Y':  // year (four digits)
                        buf = base::utils::Str::convertAndAddToBuff ( tInfo->tm_year +
                                base::consts::kYearBase, 4, buf, bufLim );
                        continue;
                    case 'h':  // hour (12-hour)
                    ...
                    case 'z':  // milliseconds
                    case 'g':
                        buf = base::utils::Str::convertAndAddToBuff ( msec, msWidth->m_width, buf,
                                bufLim );
                        continue;
                    case 'F':  // AM/PM
                        buf = base::utils::Str::addToBuff ( ( tInfo->tm_hour >= 12 ) ?
                                                            base::consts::kPm : base::consts::kAm, buf, bufLim );
                        continue;
                    default:
                        continue;
                    }
            ...
            for ( int i = 1; i < m_argc; ++i ) {
                ...
                if ( v != nullptr && strlen ( v ) > 0 ) {
                    ...
                    if ( hasParamWithValue ( key.c_str() ) ) {
                        ELPP_INTERNAL_INFO ( 1, "Skipping [" << key <<
...
class Registry : public AbstractRegistry<T_Ptr, std::map<T_Key, T_Ptr*>>
{
    ...
    private:
        virtual inline void deepCopy ( const
                                       AbstractRegistry<T_Ptr, std::map<T_Key, T_Ptr*>>& sr ) ELPP_FINAL {
...
class RegistryWithPred : public AbstractRegistry<T_Ptr, std::vector<T_Ptr*>>
{
    ...
    private:
        virtual inline void deepCopy ( const
                                       AbstractRegistry<T_Ptr, std::vector<T_Ptr*>>& sr )
        {
            for ( const_iterator it = sr.list().begin(); it != sr.list().end(); ++it ) {
...
class Loggable
{
    ...
    private:
        friend inline el::base::type::ostream_t& operator<< ( el::base::type::ostream_t&
                os, const Loggable& loggable )
        {
            loggable.log ( os );
            return os;
        }
    ...
    public:
        ...
        void parseFromFormat ( const base::type::string_t& userFormat )
        {
            ...
            base::FormatFlags flag ) {
                std::size_t foundAt = base::type::string_t::npos;
                while ( ( foundAt = formatCopy.find ( specifier,
                                                      foundAt + 1 ) ) != base::type::string_t::npos ) {
                    if ( foundAt > 0
    ...
    public:
        ...
        virtual inline void log ( el::base::type::ostream_t& os ) const
        {
            os << LevelHelper::convertToString ( m_level )
               << ELPP_LITERAL ( " " ) << ConfigurationTypeHelper::convertToString (
                   m_configurationType )
...
class Configurations : public
    base::utils::RegistryWithPred<Configuration, Configuration::Predicate>
{
    public:
        ...
        Configurations ( const std::string& configurationFile,
                         bool useDefaultsForRemaining = true, Configurations* base = nullptr ) :
        ...
        class Parser : base::StaticClass
        {
            public:
                                            ...
                                            Configurations* sender, Configurations* base = nullptr )
                {
                    ...
                    while ( fileStream_.good() ) {
                        ...
                        parsedSuccessfully = parseLine ( &line, &currConfigStr, &currLevelStr,
                                                         &currLevel, sender );
            ...
            private:
                ...
                {
                    return base::utils::Str::startsWith ( line,
                                                          std::string ( base::consts::kConfigurationLevel ) );
                }

                static inline bool isComment ( const std::string& line )
                {
                    return base::utils::Str::startsWith ( line,
                                                          std::string ( base::consts::kConfigurationComment ) );
                }
...
class TypedConfigurations : public base::threading::ThreadSafe
{
    ...
    private:
        ...
        inline Conf_T getConfigByVal ( Level level,
                                       const std::map<Level, Conf_T>* confMap, const char* confName )
        {
            base::threading::ScopedLock scopedLock ( lock() );
                                             ...
                                             const std::map<Level, Conf_T>* confMap, const char* confName )
        {
            ...
            if ( it == confMap->end() ) {
                try {
                    ...
                    ELPP_INTERNAL_ERROR ( "Unable to get configuration [" << confName <<
                                          "] for level ["
        ...
        inline Conf_T& unsafeGetConfigByRef ( Level level,
                                              std::map<Level, Conf_T>* confMap, const char* confName )
        {
            ELPP_UNUSED ( confName );
        ...
        void setValue ( Level level, const Conf_T& value,
                        std::map<Level, Conf_T>* confMap, bool includeGlobalLevel = true )
        {
        ...
        {
            base::threading::ScopedLock scopedLock ( lock() );
            auto getBool = [] ( std::string boolStr ) ->
            bool { // Pass by value for trimming
            ...
            for ( Configurations::const_iterator it = configurations->begin();
                ...
                if ( conf->configurationType() == ConfigurationType::Enabled ) {
                    ...
                    setValue ( conf->level(),
                               static_cast<std::size_t> ( getULong ( conf->value() ) ), &m_maxLogFileSizeMap );
#if !defined(ELPP_NO_DEFAULT_LOG_FILE)
        ...
        std::string resolveFilename ( const std::string& filename )
        {
            ...
            if ( ( dateIndex = resultingFilename.find (
                ...
                if ( dateIndex != std::string::npos ) {
                    ...
                    base::utils::Str::replaceAll ( resultingFilename, dateTimeFormatSpecifierStr,
                                                   now );
                }
            }
        ...
        void insertFile ( Level level, const std::string& fullFilename )
        {
            ...
            auto create = [&] ( Level level ) {
                ...
                if ( filestreamIter == m_logStreamsReference->end() ) {
                    ...
                    m_logStreamsReference->insert ( std::make_pair ( resolvedFilename,
                    ...
                    m_fileStreamMap.insert ( std::make_pair ( level,
            ...
            create ( m_filenameMap.empty()
                     && m_fileStreamMap.empty() ? Level::Global : level );
        }

        bool unsafeValidateFileRolling ( Level level,
                                         const PreRollOutCallback& PreRollOutCallback )
        {
            base::type::fstream_t* fs = unsafeGetConfigByRef ( level, &m_fileStreamMap,
                                        "fileStream" ).get();
            if ( fs == nullptr ) {
                return true;
            }
            std::size_t maxLogFileSize = unsafeGetConfigByVal ( level, &m_maxLogFileSizeMap,
                                         "maxLogFileSize" );
...
class RegisteredHitCounters : public
    base::utils::RegistryWithPred<base::HitCounter, base::HitCounter::Predicate>
{
    public:
...
{
    public:
        LogDispatchData() : m_logMessage ( nullptr ),
...
class Logger : public base::threading::ThreadSafe, public Loggable
{
    public:
        ...
        void configure ( const Configurations& configurations )
        {
            ...
            base::utils::safeDelete ( m_typedConfigurations );
            m_typedConfigurations = new base::TypedConfigurations ( &m_configurations,
        ...
        inline void setParentApplicationName ( const std::string&
    ...
    public:
        ...
        Logger* get ( const std::string& id, bool forceCreation = true )
        {
            ...
            if ( logger_ == nullptr && forceCreation ) {
                ...
                if ( !validId ) {
                    ELPP_ASSERT ( validId, "Invalid logger ID [" << id <<
...
class VRegistry : base::NoCopy, public base::threading::ThreadSafe
{
    public:
        explicit VRegistry ( base::type::VerboseLevel level,
                             base::type::EnumType* pFlags ) : m_level ( level ), m_pFlags ( pFlags )
        {
        }
...
class AsyncLogItem
{
    public:
        explicit AsyncLogItem ( const LogMessage& logMessage,
                                const LogDispatchData& data, const base::type::string_t& logLine )
...
class Storage : base::NoCopy, public base::threading::ThreadSafe
{
    public:
            ...
            m_preRollOutCallback ( base::defaultPreRollOutCallback )
        {
            ...
            Logger* performanceLogger = m_registeredLoggers->get ( std::string (
                                            base::consts::kPerformanceLoggerId ) );
            ...
            Logger* sysLogLogger = m_registeredLoggers->get ( std::string (
                                       base::consts::kSysLogLoggerId ) );
        ...
        inline bool validateAfterNCounter ( const char* filename,
                                            unsigned long int lineNumber, std::size_t n )  // NOLINT
        {
        ...
        inline void installCustomFormatSpecifier ( const CustomFormatSpecifier&
        ...
        inline bool uninstallCustomFormatSpecifier ( const char* formatSpecifier )
        {
            ...
            std::vector<CustomFormatSpecifier>::iterator it = std::find (
                        m_customFormatSpecifiers.begin(),
        ...
        {
            return installCallback<T, base::type::LogDispatchCallbackPtr> ( id,
        ...
        {
            uninstallCallback<T, base::type::LogDispatchCallbackPtr> ( id,
                    &m_logDispatchCallbacks );
        }
        template <typename T>
        inline T* logDispatchCallback ( const std::string& id )
        {
            return callback<T, base::type::LogDispatchCallbackPtr> ( id,
                    &m_logDispatchCallbacks );
        }
        ...
        {
            uninstallCallback<T, base::type::PerformanceTrackingCallbackPtr> ( id,
                    &m_performanceTrackingCallbacks );
        }
    ...
    private:
        ...
        void setApplicationArguments ( int argc, char** argv )
        {
            ...
            if ( m_commandLineArgs.hasParamWithValue (
                ...
                m_flags = atoi ( m_commandLineArgs.getParamValue (
                                     base::consts::kLoggingFlagsParam ) );
            }
#endif  // defined(ELPP_LOGGING_FLAGS_FROM_ARG)
        }
            ...
            if ( m_data->dispatchAction() == base::DispatchAction::NormalLog ) {
                if ( m_data->logMessage()->logger()->m_typedConfigurations->toFile (
                    ...
                    if ( fs != nullptr ) {
                        ...
                        ELPP_INTERNAL_ERROR ( "Log file for [" << LevelHelper::convertToString (
                                                  m_data->logMessage()->level() ) << "] "
    ...
    protected:
        void handle ( const LogDispatchData* data )
        {
            base::type::string_t logLine =
                data->logMessage()->logger()->logBuilder()->build ( data->logMessage(),
            ...
            if ( data->dispatchAction() == base::DispatchAction::NormalLog
                ...
                if ( ELPP->hasFlag ( LoggingFlag::ColoredTerminalOutput ) )
                    data->logMessage()->logger()->logBuilder()->convertToColoredOutput ( &logLine,
                            data->logMessage()->level() );
                ELPP_COUT << ELPP_COUT_LINE ( logLine );
            }
            ...
            if ( data->logMessage()->logger()->typedConfigurations()->toFile (
                        data->logMessage()->level() ) ) {
                ELPP->asyncLogQueue()->push ( AsyncLogItem ( * ( data->logMessage() ), *data,
                                              logLine ) );
            }
        }
};
...
{
    public:
        MessageBuilder ( void ) : m_logger ( nullptr ),
...
#   endif
        ...
        inline MessageBuilder& operator<< ( std::ostream & ( *OStreamMani ) (
                                                std::ostream& ) )
        {
...
#define ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG(temp)                                                   \
    ...
    inline MessageBuilder& operator<<(const temp<T1, T2, T3, T4, T5>& template_inst) {               \
        ...
        inline MessageBuilder& operator<< ( const
                                            std::priority_queue<T, Container, Comparator>& priorityQueue_ )
        {
            ...
            return writeIterator ( iterablePriorityQueue_.begin(),
                                   iterablePriorityQueue_.end(), iterablePriorityQueue_.size() );
        }
...
#undef ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG
    private:
        ...
        {
            m_logger->stream() << ELPP_LITERAL ( "[" );
            for ( std::size_t i = 0; begin_ != end_
                    && i < base::consts::kMaxLogPerContainer; ++i, ++begin_ ) {
                operator << ( *begin_ );
...
class Writer : base::NoCopy
{
    ...
    protected:
                                ...
                                bool needLock = true )
        {
            ...
            if ( m_logger == nullptr ) {
                ELPP->acquireLock();
                if ( !ELPP->registeredLoggers()->has ( std::string (
                        base::consts::kDefaultLoggerId ) ) ) {
                    // Somehow default logger has been unregistered. Not good! Register again
                    ELPP->registeredLoggers()->get ( std::string (
                                                         base::consts::kDefaultLoggerId ) );
                }
                ELPP->releaseLock();  // Need to unlock it for next writer
                Writer ( Level::Debug, m_file, m_line, m_func ).construct ( 1,
                        base::consts::kDefaultLoggerId )
                ...
                if ( ELPP->hasFlag ( LoggingFlag::HierarchicalLogging ) ) {
                    m_proceed = m_level == Level::Verbose ? m_logger->enabled ( m_level ) :
                                LevelHelper::castToInt ( m_level ) >= LevelHelper::castToInt (
                                    ELPP->m_loggingLevel );
                } else {
        ...
        void triggerDispatch ( void )
        {
            ...
            if ( m_proceed && m_level == Level::Fatal
                ...
                base::Writer ( Level::Warning, m_file, m_line, m_func ).construct ( 1,
                        base::consts::kDefaultLoggerId )
...
class PerformanceTracker : public base::threading::ThreadSafe, public Loggable
{
    public:
        PerformanceTracker ( const std::string& blockName,
                             ...
                             bool scopedLog = true, Level level =
                                 base::consts::kPerformanceTrackerDefaultLevel ) :
            m_blockName ( blockName ), m_timestampUnit ( timestampUnit ),
            m_loggerId ( loggerId ), m_scopedLog ( scopedLog ),
            m_level ( level ), m_hasChecked ( false ), m_lastCheckpointId ( std::string() ),
            m_enabled ( false )
        {
        ...
        virtual ~PerformanceTracker ( void )
        {
            ...
            if ( m_enabled ) {
                ...
                if ( m_scopedLog ) {
                    ...
                    for ( const std::pair<std::string, base::type::PerformanceTrackingCallbackPtr>&
                            h
                          ...
                          const char* func = "" )
        {
            ...
            if ( m_enabled ) {
                base::threading::ScopedLock scopedLock ( lock() );
                base::utils::DateTime::gettimeofday ( &m_endTime );
                base::type::string_t formattedTime = m_hasChecked ? getFormattedTimeTaken (
                        m_lastCheckpointTime ) : ELPP_LITERAL ( "" );
    ...
    private:
        ...
        const base::type::string_t getFormattedTimeTaken ( struct timeval startTime )
        const
        {
            ...
            return base::utils::DateTime::formatTime (
                       base::utils::DateTime::getTimeDifference ( m_endTime,
    ...
    protected:
        void handle ( const PerformanceTrackingData* data )
        {
            ...
            el::base::Writer ( m_data->performanceTracker()->level(), m_data->file(),
                               m_data->line(), m_data->func() ).construct ( 1,
...
class Helpers : base::StaticClass
{
    public:
        ...
        static inline void setCrashHandler ( const
                                             el::base::debug::CrashHandler::Handler& crashHandler )
        {
        ...
        static inline void installCustomFormatSpecifier ( const CustomFormatSpecifier&
                customFormatSpecifier )
        {
            ELPP->installCustomFormatSpecifier ( customFormatSpecifier );
        }
        /// @brief Uninstalls user defined format specifier and handler
        static inline bool uninstallCustomFormatSpecifier ( const char*
                formatSpecifier )
        {
        ...
        static inline void validateFileRolling ( Logger* logger, Level level )
        {
            ...
            logger->m_typedConfigurations->validateFileRolling ( level,
...
class Loggers : base::StaticClass
{
    public:
        ...
        static inline void reconfigureAllLoggers ( const Configurations&
                configurations )
        {
        ...
        static inline void setDefaultConfigurations ( const Configurations&
                configurations, bool reconfigureExistingLoggers = false )
        {
        ...
        static void configureFromGlobal ( const char* globalConfigurationFilePath )
        {
            ...
            auto configure = [&] ( void ) {
                ELPP_INTERNAL_INFO ( 8, "Configuring logger: '" << logger->id() <<
                                     "' with configurations \n" << ss.str()
                                     << "\n--------------" );
                Configurations c;
...
{
    CLOG_IF ( ptr == nullptr, FATAL,
              loggers ) << "Check failed: [" << name << " != nullptr]";
    return ptr;
}
}  // namespace utils
}  // namespace base
